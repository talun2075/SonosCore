...
namespace SonosUPnP.Services.MediaServerServices
{
    public class ConnectionManager
    {
        ...
        private void EventFired_SourceProtocolInfo(UPnPStateVariable sender, object NewValue)
        {
            ...
            if(pl.PlayerProperties.MS_ConnectionManager_SourceProtocolInfo != nv)
            pl.PlayerProperties.MS_ConnectionManager_SourceProtocolInfo = nv;
        }

        private void EventFired_CurrentConnectionIDs(UPnPStateVariable sender, object NewValue)
        {
            var nv = NewValue.ToString();
            pl.PlayerProperties.MS_ConnectionManager_CurrentConnectionIDs = nv;
        }

        private void EventFired_SinkProtocolInfo(UPnPStateVariable sender, object NewValue)
        {
            var nv = NewValue.ToString();
            if (pl.PlayerProperties.MS_ConnectionManager_SinkProtocolInfo != nv)
                pl.PlayerProperties.MS_ConnectionManager_SinkProtocolInfo = nv;
        }
        #endregion Eventing
        ...
        public async Task<List<String>>  GetProtocolInfo()
        {
            ...
            await ServiceWaiter.WaitWhileAsync(arguments, 0, 100, 3, WaiterTypes.String);
            return arguments[0].DataValue.ToString().Split(',').ToList();
        }
