	...
	public class StringCompressor
	{
		...
		static public int BestMatch(byte[] buffer, int Offset, int bufferLength,int WindowSize, int MinMatchSize, int MaxMatchSize, out int MatchOffset, out int MatchLength)
		{
			...
			for(int x=0;x<MinMatchSize;++x)
			{
				...
				Match = FindMatch(buffer,UseOffset+Forward,UseSize,buffer,Offset+Forward,currentLength,out int _offset);
				if (Match)
				{
					while(Offset+MinMatchSize<=bufferLength && FindMatch(buffer,UseOffset+Forward,UseSize,buffer,Offset+Forward,currentLength,out int _offset2))
					{
						_offset = _offset2;
						++currentLength;
					}
...
//						}
		...
		static private byte[] CompressString(byte[] inbuf, int inbufOffset, int inbufLength)
		{
            ...
            while (currentOffset + currentLength <= inbufLength)
			{
				...
				ln = written-i;
				if (FindMatch(inbuf,i,ln,inbuf,currentOffset,currentLength,out int Offset))
				{
