...
namespace OSTL.UPnP
{
    ...
    public sealed class UPnPDevice
    {
        ...
        private readonly Hashtable CP_RegisteredInterfaces = new();

        private readonly Hashtable UpdateTable = new();
        private readonly Hashtable CustomField = new();

        internal bool NoSSDP = false;

        private int UseThisPort = 0;
        private readonly Hashtable InitialEventTable = Hashtable.Synchronized(new Hashtable());
        ...
        private readonly Hashtable VirtualDir_Table;
        private readonly Hashtable VirtualDir_Header_Table;
        ...
        private readonly Hashtable WebServerTable;
        ...
        private readonly bool ControlPointOnly;
        ...
        /// </summary>
        public UPnPDevice[] EmbeddedDevices = Array.Empty<UPnPDevice>();
        /// <summary>
        ...
        internal UPnPDevice()
        {
            ...
            Services = Array.Empty<UPnPService>();
        ...
        private void NewCPInterface(NetworkInfo sender, IPAddress ip)
        {
            lock (typeof(UPnPDevice))
            {
                ...
                {
                    MiniWebServer ws = new(new IPEndPoint(ip, 0));
        ...
        public void UpdateDevice(Uri LocationUri, IPAddress HostInterface)
        {
            lock (UpdateTable)
            {
                UPnPDeviceFactory df = new(LocationUri, 250, UpdateDeviceSink, null, null, null);
                UpdateTable[df] = df;
            }
        }
        ...
        private void ManualNewDeviceInterface(IPAddress ip, int Port)
        {
            ...
            MiniWebServer WebServer = new(new IPEndPoint(ip, Port));
        ...
        {
            OnSniff?.Invoke(raw, offset, length);
        }
        private void SniffSessionSink3(HTTPSession sender, HTTPMessage msg)
        {
            OnSniffPacket?.Invoke((HTTPMessage)msg.Clone());
        }
        ...
        private void HandleHeaderRequest(HTTPMessage msg, HTTPSession WebSession)
        {
            ...
            if ((H_cb != null) || (P_cb != null))
            {
                ...
                H_cb?.Invoke(this, _msg, WebSession, vd);
                return;
            }
        }
            ...
            if (Method == "SUBSCRIBE")
            {
                ...
                if (Response2 != null)
                {
                    ...
                    for (int x = 0; x < cbURL.Length; ++x)
                    {
                        ...
                        IPEndPoint d = new(IPAddress.Parse(cbURL[x].Host), cbURL[x].Port);
                        HTTPRequest R = new();
        ...
        private HTTPMessage Get(String GetWhat, IPEndPoint local)
        {
            ...
            try
            {
                FileStream fs = new(RootPath + GetWhat, FileMode.Open, FileAccess.Read, FileShare.Read);
        ...
        public void AddService(UPnPService service)
        {
            if (ControlPointOnly)
            {
                ...
                MiniWebServer mws;
                String ITH;
                SSDP.ParseURL(service.__eventurl, out string WebIP, out int WebPort, out string WebData);
                if (InterfaceToHost != null)
                {
                    try
                    {
        ...
        public byte[] GetRootDeviceXML(IPEndPoint local)
        {
            ...
            XmlTextWriter XDoc = new(mstream, Encoding.UTF8);
