...
namespace OSTL.UPnP
{
    ...
    public class UPnPAction : ICloneable
    {
        ...
        {
            ArrayList OutList = new();
            ArrayList InList = new();
    ...
    public sealed class UPnPService : ICloneable
    {
        ...
        private readonly Hashtable ComplexTypeTable = Hashtable.Synchronized(new Hashtable());
        ...
        internal Hashtable SchemaURLS = new();
        ...
        public string GetComplexSchemaForNamespace(string ns)
        {
            string RetVal;
            MemoryStream m = new();
            XmlTextWriter X = new(m, Encoding.UTF8);
        ...
        private readonly WeakEvent PeriodicRenewFailedEvent = new();
        ...
        private readonly WeakEvent SubscriptionAddedEvent = new();
        private readonly WeakEvent SubscriptionRemovedEvent = new();
        ...
        private readonly Hashtable SubscribeRequestTable = Hashtable.Synchronized(new Hashtable());
        private readonly Hashtable SendEventTable = Hashtable.Synchronized(new Hashtable());
        private readonly LifeTimeMonitor.LifeTimeHandler SubscribeCycleCallback;
        public delegate void ServiceResetHandler(UPnPService sender);
        public event ServiceResetHandler OnServiceReset;
        private readonly HTTPRequest InvocationPipeline = new();
        ...
        private readonly Hashtable DelayedResponseTable = new();
        ...
        private static readonly LifeTimeMonitor SubscribeCycle = new();
        ...
        private readonly Hashtable SubscriberTable;
        private int EventSID;
        private readonly object SIDLock;

        private String CurrentSID;
        private int CurrentTimeout;
        //private System.Timers.Timer RenewalTimer;

        private readonly Hashtable VarAssociation;
        ...
        public void Subscribe(int Timeout, UPnPEventSubscribeHandler CB)
        {
            ...
            SSDP.ParseURL(__eventurl, out string WebIP, out int WebPort, out string Tag);
            IPEndPoint dest = new(IPAddress.Parse(WebIP), WebPort);
        ...
        {
            StringReader MyString = new(XML);
            XmlTextReader XMLDoc = new(MyString);
            UPnPService RetVal = new(1);
        ...
        private void ParseStateVarXml(String evented, String multicasted, XmlTextReader XMLDoc, int startLine)
        {
            ...
            ArrayList allowedValueList = new();
            string LocalName = "";
            string lname2 = "";
        ...
        public byte[] GetSCPDXml()
        {
            ...
            XmlTextWriter XDoc = new(mstr, Encoding.UTF8);
            ...
            UPnPServiceInvokeErrorHandler ErrorCallback)
        {
            ...
            if (InVarArr == null)
            {
                InVarArr = Array.Empty<UPnPArgument>();
            }
            ...
            SSDP.ParseURL(__controlurl, out string WebIP, out int WebPort, out string sName);
            IPEndPoint dest = new(IPAddress.Parse(WebIP), WebPort);
            ...
            MemoryStream mstream = new(4096);
            XmlTextWriter W = new(mstream, Encoding.UTF8);
        ...
        private void HandleInvokeRequest(HTTPRequest sender, HTTPMessage response, object Tag)
        {
            ...
            if (response == null)
            {
                ...
                {
                    OnInvokeError?.Invoke(this, state.MethodName, state.Args, new UPnPInvokeException(state.MethodName, state.Args, "Could not connect to device"), state.Tag);
                }
                return;
            }
            ...
            if (response.StatusCode != 200)
            {
                if ((OnInvokeError != null) || (state.ErrorCB != null))
                {
                    if ((response.StatusCode == 500) && (response.BodyBuffer.Length > 0))
                    {
                        ...
                        {
                            OnInvokeError?.Invoke(this, action.Name, state.Args, new UPnPInvokeException(state.MethodName, state.Args, response.StatusData, ce), state.Tag);
                        }
                    }
                    else
                    {
                        ...
                        {
                            OnInvokeError?.Invoke(this, action.Name, state.Args, new UPnPInvokeException(state.MethodName, state.Args, response.StatusData), state.Tag);
                        }
                    }
                }
                return;
            ...
            StringReader MyString = new(response.StringBuffer.Trim());
            XmlTextReader XMLDoc = new(MyString);

            String MethodTag;
            UPnPArgument VarArg;
            ArrayList VarList = new();
            ...
            {
                OnInvokeResponse?.Invoke(this, state.MethodName, InVarArr, RetVal, state.Tag);
            }
        }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (ci != null)
                                {
                                    try
                                    {
                                        RetObj = ci.Invoke(Array.Empty<object>());
                                    }
                                    catch (Exception)
                                    {
                                        throw (new Exception("Could not instantiate " + ObjectType.FullName));
        ...
        internal object InvokeLocal(String MethodName, ref ArrayList VarList)
        {
            ...
            if (A.SpecialCase != null)
            {
                ...
                A.SpecialCase(A, (UPnPArgument[])VarList.ToArray(typeof(UPnPArgument)), out object _RetArg, out UPnPArgument[] OutArgs);
                VarList.Clear();
                ...
                if (A.HasReturnValue)
                {
                    UPnPArgument retArg = new(A.GetRetArg().Name, _RetArg);
            ...
            if (A.HasReturnValue)
            {
                UPnPArgument retArg = new(A.GetRetArg().Name, RetVal);
                retArg.IsReturnValue = true;
                RetVal = retArg;
            }
            return RetVal;
        ...
        {
            MemoryStream ms = new();
            XmlTextWriter XMLDoc = new(ms, Encoding.UTF8);
